...
county_df <- read.csv(file='data/acs2017_county_data.csv')
...
# out the data frame.
up_counties_df <- up_counties[[1]]

# Now let's get rid of the Total line and get the county names into a vector.
up_counties_df <- up_counties_df %>%
  filter(County != 'Total')

up_counties_v <- up_counties_df$County
...
mi_counties <- mi_counties %>%
  mutate(peninsula = if_else(County %in% up_counties_v, "UP", "LP"))
...
# Clean up
rm(mi_counties, up_counties, county_df, url, 
   county_MI, states, new_col_order, up_counties_df, up_counties_v)
